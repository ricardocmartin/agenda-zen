...
export default {
  ...
  async mounted() {
    ...
    const { data: clients } = await supabase.from('clients').select().in('user_id', this.userIds)
    if (clients) this.clients = clients

    const { data: services } = await supabase.from('services').select().in('user_id', this.userIds)
      .eq('active', true)
